...
RasterizeGaussiansCUDA(
	...
	const float pixel_cx,
	const float pixel_cy,
	const bool debug)
{
  ...
  torch::Tensor out_peak_index = torch::full({1, H, W}, 0, means3D.options().dtype(torch::kInt32));
  torch::Tensor out_peak_weight = torch::full({1, H, W}, 0, float_opts);
  ...
  if(P != 0)
  {
	  ...
	  rendered = CudaRasterizer::Rasterizer::forward(
		...
		out_peak_index.contiguous().data<int>(),
		out_peak_weight.contiguous().data<float>(),
		render_geo,
		pixel_cx,
	    pixel_cy,
		debug);
  }
  return std::make_tuple(rendered, out_color, radii, out_observe, out_all_map, out_plane_depth, out_peak_index, out_peak_weight, geomBuffer, binningBuffer, imgBuffer);
}
