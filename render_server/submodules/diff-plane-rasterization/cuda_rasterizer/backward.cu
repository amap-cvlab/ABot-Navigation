...
// DISTWAR - backward kernel with serialized atomic reduction (SW-S)
template <uint32_t C,uint32_t MAP_N>
__global__ void __launch_bounds__(BLOCK_X * BLOCK_Y)
renderCUDABW_serialized(
	const uint2* __restrict__ ranges,
	const uint32_t* __restrict__ point_list,
	int W, int H,
	float fx, float fy,
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			if (render_geo) {
				for (int ch = 0; ch < MAP_N; ch++)
				{
					...
					// DISTWAR - cache all_map updates
				    dall_map[ch] = dchannel_dcolor * dL_dchannel; // new
				}
			}
			...
			float dmeanX_abs = fabs(dL_dG * dG_ddelx * ddelx_dx); // new
			float dmeanY_abs = fabs(dL_dG * dG_ddely * ddely_dy); // new
            ...
            if (render_geo){
                float *atom_ptrs[16] = {
                    ...
                    &dL_dmean2D_abs[global_id].x, // new
                    &dL_dmean2D_abs[global_id].y, // new
                    ...
                    &dL_dall_map[global_id * MAP_N + 0], // new
                    &dL_dall_map[global_id * MAP_N + 1], // new
                    &dL_dall_map[global_id * MAP_N + 2], // new
                    &dL_dall_map[global_id * MAP_N + 3], // new
                    &dL_dall_map[global_id * MAP_N + 4], // new
                };
			    ...
			    atomred_vec(global_id, atom_ptrs, atom_vals, 16, balance_threshold);
            }else{
                float *atom_ptrs[11] = {
                    &dL_dmean2D[global_id].x,
                    &dL_dmean2D[global_id].y,
                ...
                // DISTWAR - perform all atomic updates using serialized atomic reduction (SW-S)
			    atomred_vec(global_id, atom_ptrs, atom_vals, 11, balance_threshold);
            }
...
// DISTWAR - backward kernel with butterfly atomic reduction (SW-B)
template <uint32_t C,uint32_t MAP_N>
__global__ void __launch_bounds__(BLOCK_X * BLOCK_Y)
renderCUDABW_butterfly(
	const uint2* __restrict__ ranges,
	const uint32_t* __restrict__ point_list,
	int W, int H,
	float fx, float fy,
	...
	unsigned balance_threshold)
{
	...
	float accum_rec[C] = { 0 };  // C 3
	float accum_all_map[MAP_N] = { 0 };  // MAP_N 5
	float dL_dpixel[C];
	float dL_dout_all_map[MAP_N];

    // std::printf("CUDA DEBUG C: %f", static_cast<float>(C));
    // std::printf("CUDA DEBUG MAP_N: %f", static_cast<float>(MAP_N));
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			float dmeanX_abs = skip ? 0.0f : fabs(dL_dG * dG_ddelx * ddelx_dx); // new
			float dmeanY_abs = skip ? 0.0f : fabs(dL_dG * dG_ddely * ddely_dy); // new
			...
			if (__match_any_sync(__activemask(), global_id) == 0xFFFFFFFF && active_ct >= balance_threshold) {
                ...
                for (int offset = 16; offset >= 1; offset /= 2) {
                    ...
                    dmeanX_abs += __shfl_down_sync(0xFFFFFFFF, dmeanX_abs, offset); // new
                    dmeanY_abs += __shfl_down_sync(0xFFFFFFFF, dmeanY_abs, offset); // new
                    ...
                    if (render_geo) {
                        dall_map[0] += __shfl_down_sync(0xFFFFFFFF, dall_map[0], offset); // new
                        dall_map[1] += __shfl_down_sync(0xFFFFFFFF, dall_map[1], offset); // new
                        dall_map[2] += __shfl_down_sync(0xFFFFFFFF, dall_map[2], offset); // new
                        dall_map[3] += __shfl_down_sync(0xFFFFFFFF, dall_map[3], offset); // new
                        dall_map[4] += __shfl_down_sync(0xFFFFFFFF, dall_map[4], offset); // new
                    }
                }
                ...
                if (laneId == 0) {
                    ...
                    atomicAdd(&dL_dmean2D_abs[global_id].x, dmeanX_abs); // add
                    atomicAdd(&dL_dmean2D_abs[global_id].y, dmeanY_abs); // add
                    ...
                    if (render_geo) {
                        atomicAdd(&(dL_dall_map[global_id * MAP_N + 0]), dall_map[0]); // new
                        atomicAdd(&(dL_dall_map[global_id * MAP_N + 1]), dall_map[1]); // new
                        atomicAdd(&(dL_dall_map[global_id * MAP_N + 2]), dall_map[2]); // new
                        atomicAdd(&(dL_dall_map[global_id * MAP_N + 3]), dall_map[3]); // new
                        atomicAdd(&(dL_dall_map[global_id * MAP_N + 4]), dall_map[4]); // new
                    }
                }
            } else if (!skip) {		// DISTWAR - SW-B condition not met, update gradients normally
                atomicAdd(&(dL_dmean2D[global_id].x), dmeanX);
                atomicAdd(&(dL_dmean2D[global_id].y), dmeanY);
                atomicAdd(&(dL_dmean2D_abs[global_id].x), dmeanX_abs); // new
                atomicAdd(&(dL_dmean2D_abs[global_id].y), dmeanY_abs); // new
	...
	const bool render_geo)
{
	...
	switch(bw_impl) {
		case 0:
		if (ct == 1) {
			std::cout << "(PGSR-FIX) BW IMPLEMENTATION: ORIGINAL (no optimization)\n";
		}
		renderCUDABW_original<NUM_CHANNELS,NUM_ALL_MAP> <<<grid, block >>>(
			ranges,
			point_list,
			W, H,
			fx, fy,
		...
		case 1:
		if (ct == 1) {
			std::cout << "(PGSR-FIX) BW IMPLEMENTATION: BUTTERFLY (SW-B)\n";
		}
		renderCUDABW_butterfly<NUM_CHANNELS,NUM_ALL_MAP> <<<grid, block >>>(
			ranges,
			point_list,
			W, H,
			fx, fy,
		...
		case 2:
		if (ct == 1) {
			std::cout << "(PGSR-FIX) BW IMPLEMENTATION: SERIALIZED (SW-S)\n";
		}
		renderCUDABW_serialized<NUM_CHANNELS,NUM_ALL_MAP> <<<grid, block >>>(
			ranges,
			point_list,
			W, H,
			fx, fy,
		...
		"1: SW-B butterfly atomic reduction\n" <<
		"2: SW-S serialized atomic reduction" << std::endl;
		assert(0);
		break;
	}
	if (ct == 1) {
